...
# Arm-only controller only needs the chain up to RightHand.
LINKS_PARENT = {
    "Hips": "world",
    "Spine": "Hips",
    "Spine1": "Spine",
...
class Fr3RvizArmController:
    def __init__(
        ...
        self._node = rclpy.create_node("fr3_rviz_arm_controller")
    ...
    def run(self) -> None:
        ...
        self._node.get_logger().info(
            f"Running mocap->FR3 RViz arm controller, publish_topic={self._publish_topic}"
        )
        try:
            while rclpy.ok():
                t0 = time.time()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Mocap controls FR3 arm in RViz via /joint_states")
...
def main():
    ...
    controller = Fr3RvizArmController(
        urdf_path=urdf_path,
