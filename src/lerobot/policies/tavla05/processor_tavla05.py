...
"""
TA-VLA05 specific data processors.
...
class Tavla05InputProcessor:
    """Processes inputs for TA-VLA05 model.
    ...
    def __init__(self, config: Tavla05Config):
        self.config = config
    ...
    def __call__(self, batch: dict[str, Any]) -> dict[str, Any]:
        """Process input batch for TA-VLA05 model.
...
class Tavla05OutputProcessor:
    """Processes outputs from TA-VLA05 model.
    ...
    def __call__(self, batch: dict[str, Any]) -> dict[str, Any]:
        """Process output batch from TA-VLA05 model.
...
def make_tavla05_pre_post_processors(
    config: Tavla05Config,
    **kwargs,
) -> tuple[PolicyProcessorPipeline, PolicyProcessorPipeline]:
    """Create pre- and post-processors for TA-VLA05 policy.
    
    Args:
        config: TA-VLA05 configuration
        **kwargs: Additional arguments (unused for now)
        
    Returns:
        Tuple of (preprocessor, postprocessor) pipelines
    """
    preprocessor = PolicyProcessorPipeline([Tavla05InputProcessor(config)])
    postprocessor = PolicyProcessorPipeline([Tavla05OutputProcessor(config)])
