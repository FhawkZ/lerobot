...
"""
TA-VLA (Task-Aware Vision-Language-Action) Policy for LeRobot based on PI0.5.
...
- Adds state_proj layer (PI0.5 doesn't have it by default)
...
class TavlaPytorchPI05(PI05Pytorch):
    """
    TA-VLA PyTorch model extending PI0.5 with effort/torque support.
    
    This class adds:
    - state_proj layer (PI0.5 doesn't have it by default, but TA-VLA needs it)
    - effort processing layers
    - effort tokens integration into prefix/suffix embeddings
    - Uses PI0.5's AdaRMS and time_mlp for time conditioning
    """
    
    def __init__(self, config: Tavla05Config, rtc_processor=None):
        # Convert TavlaConfig to PI05Config for base initialization
        ...
        # Initialize base PI05Pytorch
        super().__init__(pi05_config, rtc_processor=rtc_processor)
        ...
        # Add state_proj layer (PI0.5 doesn't have it, but TA-VLA needs it)
        self.state_proj = nn.Linear(config.max_state_dim, expert_width)
        ...
        if self.effort_type in (EffortType.EXPERT_FUT, EffortType.EXPERT_HIS_C_FUT, EffortType.EXPERT_HIS_C_L_FUT):
            # Replace the base action projections
    ...
    def embed_suffix(
        ...
        This method extends PI0.5's embed_suffix to include:
        - state_proj (PI0.5 doesn't have it by default)
        - effort tokens
        - Uses PI0.5's time_mlp and AdaRMS conditioning
        """
        embs = []
        pad_masks = []
        att_masks = []
        
        # Process effort tokens (before state if needed)
        ...
        # Project state (PI0.5 doesn't have state_proj, but TA-VLA needs it)
        ...
        # Embed timestep using sine-cosine positional encoding (PI0.5 style)
        ...
        # PI0.5 style: time_mlp for AdaRMS conditioning (not concatenation)
        ...
        action_time_emb = action_emb  # PI0.5 doesn't concatenate time with actions
        adarms_cond = time_cond  # Use time condition for AdaRMS
        ...
        # Process effort tokens (after actions if needed)
    ...
    def forward(
        ...
        def forward_func(prefix_embs, suffix_embs, att_2d_masks_4d, position_ids, adarms_cond):
            (_, suffix_out), _ = self.paligemma_with_expert.forward(
                ...
                use_cache=False,
                adarms_cond=[None, adarms_cond],  # PI0.5 uses AdaRMS
            )
            return suffix_out
...
class TavlaPolicyPI05(PreTrainedPolicy):
    """
    TA-VLA Policy for LeRobot based on PI0.5.
    
    This policy wraps PI0.5 with TA-VLA-specific data processing:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Convert TavlaConfig to PI05Config for the underlying model
        pi05_config = PI05Config(
            paligemma_variant=config.paligemma_variant,
            action_expert_variant=config.action_expert_variant,
            dtype=config.dtype,
        ...
        # Initialize the underlying TA-VLA model with effort support (PI0.5 based)
        # Initialize RTC processor if configured
        self.rtc_processor = None
        if pi05_config.rtc_config is not None:
            from lerobot.policies.rtc.modeling_rtc import RTCProcessor
            self.rtc_processor = RTCProcessor(pi05_config.rtc_config)
        
        # Use TavlaPytorchPI05 instead of TavlaPytorch
        self.model = TavlaPytorchPI05(config, rtc_processor=self.rtc_processor)
    ...
    def _convert_tavla_to_model_batch(self, batch: dict[str, Tensor | dict]) -> dict[str, Tensor | dict]:
        """
        Convert TA-VLA format batch to model format.
        """
        model_batch = {}
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Load a pretrained TA-VLA model based on PI0.5.
        
        Note: TA-VLA models are compatible with PI0.5 checkpoints,
        but may require weight remapping if the architecture differs.
        """
        logger.info(
            "TA-VLA model is based on PI0.5 architecture. "
            "Loading pretrained weights from PI0.5 checkpoint..."
        )
        ...
        try:
            pi05_model = PI05Policy.from_pretrained(
                pretrained_name_or_path=pretrained_name_or_path,
                config=None,
                force_download=force_download,
                resume_download=resume_download,
            ...
            model.model.load_state_dict(pi05_model.model.state_dict(), strict=strict)
            logger.info("Successfully loaded pretrained weights from PI0.5 checkpoint")
